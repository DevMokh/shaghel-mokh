...
import('./modes.js').then(m => {
  window.openRoulette      = m.openRoulette;
  window.closeRoulette     = m.closeRoulette;
  window.spinRoulette      = m.spinRoulette;
  window.startRouletteGame = m.startRouletteGame;
  window.openFlashcards    = m.openFlashcards;
  window.renderFC          = m.renderFC;
  window.flipFC            = m.flipFC;
  window.markFC            = m.markFC;
  window.restartFC         = m.restartFC;
  window.open1v1           = m.open1v1;
  window.close1v1          = m.close1v1;
  window.create1v1         = m.create1v1;
  window.join1v1           = m.join1v1;
  window._1v1Answer        = m._1v1Answer;
  console.log('[Modes] أوضاع اللعب الجديدة جاهزة ✅');
}).catch(e => {
  console.warn('[Modes] فشل تحميل أوضاع اللعب:', e);
  // fallbacks بسيطة
  window.openRoulette   = () => window.showToast('🎰 قريباً!');
  window.openFlashcards = () => window.showToast('🃏 قريباً!');
  window.open1v1        = () => window.showToast('⚔️ قريباً!');
});
...
function _selectMode(mode) {
  ...
  const _hint = document.getElementById('gm-start-hint');
...
// ══════════════════════════════════════════════════════════════════
// ربط جميع دوال الغرف الجماعية بالـ Global Window
// ══════════════════════════════════════════════════════════════════
import {
  createRoom, 
  joinRoomByCode, 
...
// دوال فتح وقفل مودال الانضمام للغرف
